...
| **2. 웨이퍼 패턴** ★ | 어떤 패턴 | MixedWM38(합성)+WM-811K(실) | 멀티라벨 분류·전이·자기지도 | 합성 0.985 → **전이 0.364** → 실데이터 강화+보정 **0.902±0.008**(macro-F1) |
...
| **+ per-class 임계보정(val-only)** | **0.902 ± 0.008** | — | ★ 최종(3-seed 실측) |

- 성능 최대화: 회전/플립 증강 + 용량↑ + leak-free 임계보정 → **Loc(병목) 0.72→0.81**, Scratch 0.78→0.80
- (negative) 종횡비 패딩·해상도↑·class-balanced 샘플링은 모두 **효과 없음**(맵이 저해상도·ASL이 이미 불균형 처리) — 정직 보고
