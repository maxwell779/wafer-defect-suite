...
> **실데이터 학습 + 구조적 개선(SE-ResNet)으로 0.93까지** 끌어올린다. 단순 튜닝·자기지도·전이는 효과 없음을 정직하게 보고한다.
...
| **2. 웨이퍼 패턴** ★ | 어떤 패턴 | MixedWM38(합성)+WM-811K(실) | 멀티라벨 분류·전이·SE-ResNet | 합성 0.985 → **전이 0.364** → SE-ResNet 앙상블 **0.929**(macro-F1) |
...
| **SE-ResNet (깊이+SE attention)** | 0.912 → 보정 **0.928** | 0.974 | ★★ 돌파 |
| **SE-ResNet 앙상블(3)+보정** | **0.929** | — | ★ 최종 best |

- 핵심: **구조적 변화(깊이+SE attention)** 가 결정적(0.86→0.93). Loc 0.72→0.85, Scratch 0.78→0.86, Near-full→1.0
- per-class(앙상블+보정): Edge-Ring 0.996 / NF 1.0 / Center 0.97 / Donut·Edge-Loc 0.93 / Random 0.92 / Scratch 0.86 / Loc 0.85
- (negative, 정직) 입력 패딩·해상도↑·class-balanced·**SimCLR·MAE·DINOv2** 모두 효과 없음 → "구조 > 단순튜닝" 입증
