...
## 8. 직전 멈춘 지점 / 다음 할 일
B(실데이터 학습) 완료. 사용자 지시 "다 해보자" → 남은 것:
- **A (빠른 진단)**: 합성 학습에 실제같은 배경노이즈+스케일 증강 추가 → 전이 재평가로 "노이즈가 갭 원인" 인과 확인
- **C (자기지도)**: WM-811K 63.8만 미라벨로 대조학습/MAE 사전학습 → 파인튜닝, 희귀클래스(Loc/Scratch/NF) boost
- (이후) 스테이지1 Meruva 공정센서 AD, 스테이지3 ELLIMAC 검출
