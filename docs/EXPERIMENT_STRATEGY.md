...
| ELLIMAC 의미 | 실제 칩표면 사진(웨이퍼맵과 다른 도메인) — 검출 스킬 데모용 | 부록 유지, 한계 명시 |
...
### ELLIMAC (실제 칩표면 사진·다른 도메인, Stage3)
- Roboflow 실제 칩표면 사진(증강)·폴리곤 라벨·cls6 노이즈. 웨이퍼맵과 다른 도메인이라 우리 과제 일반화 제한. 검출 스킬 데모용.
