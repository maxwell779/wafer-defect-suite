...
| 실험 | 결과 | 판정 |
|---|---|---|
| **#1 종횡비 패딩 + 해상도 64** | 0.880 → **0.867**(패딩) / 0.872(해상도만) | ❌ **효과 없음** — 맵이 원래 저해상도(26~52), 패딩은 die가 프레임서 작아져 손해. 꽉채우는 size52 resize가 최선 |
| **#2 class-balanced sampling** | 0.880 → **0.853** | ❌ **효과 없음** — 희귀 recall↑이나 흔한클래스 precision↓ net 손해. ASL이 이미 불균형 처리 |
| **#3 lot-split 3-seed + per-class 임계보정** | raw **0.890±0.007** → 보정 **0.902±0.008**(실측) | ✅ 신뢰구간 확정, 보정이 유일한 안정 이득(+0.01) |

**핵심 교훈**: "당연해 보이는" 입력·샘플링 개선이 **데이터로 보면 효과 없음** — 현 CNN+ASL+증강+보정이 이 데이터의 실용 천장(~0.90)에 근접. 더 올리려면 **다른 아키텍처(ResNet/attention)·자기지도 재설계(MAE)** 같은 구조적 변화가 필요(연구성 #4,#7). Stage1·#1·#2·C(자기지도)의 negative들이 "정직한 실험"의 핵심 자산.
