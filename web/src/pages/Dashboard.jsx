        ...
        <Card title={"결함 큐"} sub="이상점수순 · 행 클릭 → 우측 리포트">
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            ...
            <select value={qFilter} onChange={(e) => setQFilter(e.target.value)} aria-label="상태 필터">
              <option value="ALL">{"상태 전체"}</option><option value="미처리">{"미처리"}</option>
              <option value="격리">격리</option><option value="재검사">재검사</option><option value="보정">보정</option><option value="완료">완료</option>
            </select>
          </div>
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            <button className="btn" style={{ flex: 1 }} onClick={exportCSV} aria-label="결함 큐 CSV 내보내기">⬇ {"CSV 내보내기"}</button>
            <button className="btn" style={{ flex: 1 }} onClick={() => window.print()} aria-label="PDF 인쇄">🖨 {"PDF/인쇄"}</button>
          </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <Card title={"처리 이력"} sub={`이번 세션 조치 ${log.length}건`}>
          {log.length === 0
            ...
            : <>
                <button className="btn" style={{ marginBottom: 10 }} onClick={exportLogCSV}>⬇ {"CSV 내보내기"}</button>
